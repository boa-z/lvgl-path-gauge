...
/**
 * @brief Validates path structure without measuring any geometry.
 *
 * Rules: path non-NULL, cmd_count > 0, cmds non-NULL, the first command
 * is MOVE, and every opcode is known.
 *
 * @param[in] path  Path to validate.
 * @return          PG_OK if structurally valid;
 *                  PG_ERR_INVALID_ARG if path is NULL;
 *                  PG_ERR_INVALID_PATH otherwise.
 *
