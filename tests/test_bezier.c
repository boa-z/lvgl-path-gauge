...
/* Bezier evaluate / derivative / split. */
#include "test_util.h"
...
int main(void)
{
    ...
    /* Quad derivative at t=0.5 is (100,0). */
    ...
    /* Quad split: continuity + endpoint preservation. */
    ...
    p = pg_quad_eval(ql.p0, ql.p1, ql.p2, 0.25f);
    TU_POINT_NEAR(p,
                  pg_quad_eval(pt(0.0f, 0.0f), pt(50.0f, 100.0f),
                               pt(100.0f, 0.0f), 0.125f).x,
                  pg_quad_eval(pt(0.0f, 0.0f), pt(50.0f, 100.0f),
                               pt(100.0f, 0.0f), 0.125f).y,
                  1e-4f);
