...
static uint16_t count_written(const float *distances, uint16_t capacity)
{
    uint16_t n = 0;

    while (n < capacity && distances[n] >= 0.0f) {
        n++;
    }
    return n;
}

/** Renders at full value and counts progress pixels (is a path installed?). */
static unsigned progress_pixels(lv_test_display_t *td, lv_obj_t *gauge)
{
    lv_path_gauge_set_value(gauge, 100);
    lv_test_render(td, lv_screen_active());
    return lv_test_count_color(td, PROGRESS_COLOR, COLOR_TOL);
}
...
int main(void)
{
    ...
    TU_EXPECT(progress_pixels(&td, gauge) == 0u); /* no path installed yet */
    ...
    TU_EXPECT(res == PG_OK);
    count = count_written(g_distances, LV_PATH_GAUGE_MAX_VERTICES);
    TU_EXPECT(count > 2u);
    ...
    TU_NEAR(g_ws.tolerance, 0.25f, 1e-6f);
    TU_EXPECT(progress_pixels(&td, gauge) > 0u); /* path installed */
    ...
    TU_EXPECT(progress_pixels(&td, gauge) == 0u);
    ...
    TU_EXPECT(lv_path_gauge_set_path(gauge, &g_s_path, &g_ws) == PG_OK);
    {
        uint16_t big_count = count_written(g_distances, LV_PATH_GAUGE_MAX_VERTICES);
        float big_total = g_distances[big_count - 1u];

        TU_EXPECT(big_count > 2u && big_total > 100.0f);
        /* Distinct colours so each gauge can be counted independently. */
        ...
        lv_obj_set_style_line_width(gauge_b, 8, LV_PART_INDICATOR);
        lv_obj_set_style_line_color(gauge_b, PROGRESS_COLOR, LV_PART_INDICATOR);
        lv_obj_set_style_line_opa(gauge_b, LV_OPA_COVER, LV_PART_INDICATOR);
        for (i = 0; i < 8u; i++) {
            g_small_distances[i] = -1.0f;
        }
        TU_EXPECT(lv_path_gauge_set_path(gauge_b, &g_line_path, &g_small_ws) ==
                  PG_OK);
        {
            uint16_t small_count = count_written(g_small_distances, 8);

            TU_EXPECT(small_count > 1u);
            TU_NEAR(g_small_distances[small_count - 1u], 80.0f, 0.5f);
        }
        /* The big S curve does not fit the small workspace ... */
        TU_EXPECT(lv_path_gauge_set_path(gauge_b, &g_s_path, &g_small_ws) ==
                  PG_ERR_WORKSPACE_TOO_SMALL);
        TU_EXPECT(progress_pixels(&td, gauge_b) == 0u);
        /* ... and the small gauge recovers while the big one is untouched. */
        TU_EXPECT(lv_path_gauge_set_path(gauge_b, &g_line_path, &g_small_ws) ==
                  PG_OK);
        TU_NEAR(g_distances[big_count - 1u], big_total, 1e-4f);
        ...
        TU_EXPECT(lv_test_count_color(&td, PROGRESS_COLOR, 8) > 0u);
    }
    ...
    /* Argument validation for the remaining public API surface. */
