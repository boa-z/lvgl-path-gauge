/**
 * @file test_writer.c
 * @brief pg_path_writer_t contract and the hardening of pg_path_buffer_t.
 *
...
int main(void)
{
    ...
    /* init validates both arguments. */
    TU_EXPECT(pg_path_buffer_init(NULL, g_cmds, BUF_CAP) == PG_ERR_INVALID_ARG);
    TU_EXPECT(pg_path_buffer_init(&buffer, g_cmds, BUF_CAP) == PG_OK);
    TU_EXPECT(buffer.cmds == g_cmds && buffer.capacity == BUF_CAP &&
              buffer.count == 0u && !buffer.poisoned &&
              buffer.failure == PG_OK);

    /* NULL storage is safe: poisoned with INVALID_ARG, never crashes. */
    TU_EXPECT(pg_path_buffer_init(&buffer, NULL, BUF_CAP) ==
              PG_ERR_INVALID_ARG);
    TU_EXPECT(buffer.poisoned && buffer.failure == PG_ERR_INVALID_ARG);
    writer = pg_path_buffer_writer(&buffer);
    TU_EXPECT(writer.move_to != NULL);
    TU_EXPECT(writer.move_to(&buffer, p) == PG_ERR_INVALID_ARG);

    /* Zero capacity is likewise unusable but safe. */
    TU_EXPECT(pg_path_buffer_init(&buffer, g_cmds, 0u) ==
              PG_ERR_INVALID_ARG);
    TU_EXPECT(buffer.poisoned && buffer.failure == PG_ERR_INVALID_ARG);

    /* Writer for a NULL buffer is empty; consumers must reject it. */
    writer = pg_path_buffer_writer(NULL);
    TU_EXPECT(writer.move_to == NULL && writer.line_to == NULL &&
              writer.quad_to == NULL && writer.cubic_to == NULL &&
              writer.ctx == NULL);

    /* Healthy buffer round trip: MOVE + LINE + QUAD + CUBIC. */
    TU_EXPECT(pg_path_buffer_init(&buffer, g_cmds, BUF_CAP) == PG_OK);
    writer = pg_path_buffer_writer(&buffer);
    TU_EXPECT(writer.move_to != NULL && writer.line_to != NULL &&
              writer.quad_to != NULL && writer.cubic_to != NULL &&
    ...
    TU_EXPECT(buffer.count == 4u && !buffer.poisoned);
    ...
    /* Capacity exhaustion poisons the buffer with the capacity code and is
     * sticky; nothing is silently dropped and nothing can be exported. */
    TU_EXPECT(pg_path_buffer_init(&buffer, g_cmds, 2u) == PG_OK);
    writer = pg_path_buffer_writer(&buffer);
    TU_EXPECT(writer.move_to(&buffer, (pg_point_t){ 0.0f, 0.0f }) == PG_OK);
    ...
    TU_EXPECT(buffer.count == 2u && buffer.poisoned &&
              buffer.failure == PG_ERR_WORKSPACE_TOO_SMALL);
    TU_EXPECT(writer.line_to(&buffer, (pg_point_t){ 3.0f, 0.0f }) ==
              PG_ERR_WORKSPACE_TOO_SMALL);
    TU_EXPECT(buffer.count == 2u);
    /* to_path reports the ORIGINAL failure code and clears the output. */
    path.cmds = g_cmds;
    path.cmd_count = 7u;
    TU_EXPECT(pg_path_buffer_to_path(&buffer, &path) ==
              PG_ERR_WORKSPACE_TOO_SMALL);
    TU_EXPECT(path.cmds == NULL && path.cmd_count == 0u);

    /* A leading draw command poisons too (and reports INVALID_ARG later). */
    TU_EXPECT(pg_path_buffer_init(&buffer, g_cmds, BUF_CAP) == PG_OK);
    writer = pg_path_buffer_writer(&buffer);
    TU_EXPECT(writer.line_to(&buffer, p) == PG_ERR_INVALID_ARG);
    TU_EXPECT(buffer.poisoned && buffer.failure == PG_ERR_INVALID_ARG &&
              buffer.count == 0u);
    TU_EXPECT(pg_path_buffer_to_path(&buffer, &path) == PG_ERR_INVALID_ARG);
    TU_EXPECT(path.cmds == NULL && path.cmd_count == 0u);

    /* Non-finite coordinates poison as well. */
    TU_EXPECT(pg_path_buffer_init(&buffer, g_cmds, BUF_CAP) == PG_OK);
    writer = pg_path_buffer_writer(&buffer);
    TU_EXPECT(writer.move_to(&buffer, (pg_point_t){ 0.0f, 0.0f }) == PG_OK);
    p.x = tu_nan();
    ...
    TU_EXPECT(buffer.poisoned && buffer.failure == PG_ERR_INVALID_ARG);
    p.x = 1.0f;
    TU_EXPECT(writer.line_to(&buffer, p) == PG_ERR_INVALID_ARG); /* sticky */
    TU_EXPECT(pg_path_buffer_to_path(&buffer, &path) == PG_ERR_INVALID_ARG);
    TU_EXPECT(path.cmds == NULL && path.cmd_count == 0u);

    /* Empty (healthy) buffer yields no path, output cleared. */
    TU_EXPECT(pg_path_buffer_init(&buffer, g_cmds, BUF_CAP) == PG_OK);
    path.cmds = g_cmds;
    path.cmd_count = 3u;
